...
//    	  		mainPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, codePane, messageArea);
	...
	void switchContext(int ix) {
		...
		if (ix >= contexts.size()) {
			RunTime.terminate(999, "IDE: switchPane: invalid tab index: %d (valid: 0 .. %d)", ix, contexts.size() - 1);
		}
	...
	public File selectFileForSave(PaneContext context) {
		File fileSelected = new SikulixFileChooser(sikulixIDE).saveAs(context.getExt(),
				context.isBundle() || context.isTemp());
		if (fileSelected == null) {
			return null;
		}
		return fileSelected;
	...
	class PaneContext {
		...
		boolean doClose(boolean discard) {
			...
			if (isDirty() || (isTemp() && !discard)) {
				if (isTemp()) {
					...
					final int answer = SXDialog.askForDecision(sikulixIDE, "Closing Tab", msg, "Discard", "Save");
					if (answer == SXDialog.DECISION_CANCEL) {
						return false;
					}
					if (answer == SXDialog.DECISION_ACCEPT) {
		...
		public boolean saveAs() {
			...
			if (!isTemp() && isDirty()) {
				...
				final int answer = SXDialog.askForDecision(sikulixIDE, "Saving Tab", msg, "Do not save", "Save");
				if (answer == SXDialog.DECISION_CANCEL) {
					return false;
				}
			...
			while (true) {
				...
				if (pos >= 0) {
					...
					final int answer = SXDialog.askForDecision(sikulixIDE, "Saving Tab", msg, "Overwrite", "Try again");
					if (answer == SXDialog.DECISION_CANCEL) {
						return false;
					}
					if (answer == SXDialog.DECISION_ACCEPT) {
		...
		private void copyContent(PaneContext currentContext, PaneContext newContext, boolean asBundle)
				throws IOException {
			if (asBundle) {
			...
			if (getShowThumbs()) {
				...
				if (images.size() > 0 || patterns.size() > 0) {
					for (Map<String, Object> item : images) {
						...
						int itemStart = getLineStart((Integer) item.get(IButton.LINE))
								+ (Integer) item.get(IButton.LOFF);
		...
		public boolean resetContentToText() {
			InputStreamReader isr;
			try {
				isr = new InputStreamReader(new ByteArrayInputStream(pane.getText().getBytes(Charset.forName("utf-8"))),
						Charset.forName("utf-8"));
...
//    }
	...
	public void exportAsZip() {
		...
		if (zipFile.exists()) {
			int answer = JOptionPane.showConfirmDialog(null, SikuliIDEI18N._I("msgFileExists", zipFile),
	...
	private void parseforImagesWalk(File imageFolder, Lexer lexer, String text, int pos,
					...
					SX.popError(
							String.format(
									"Orphan string delimiter (\" or ')\n" + "in line %d\n"
											+ "No images will be deleted!\n" + "Correct the problem before next save!",
									lineNumber),
							"Delete images on save");
					error("DeleteImagesOnSave: No images deleted, caused by orphan string delimiter (\" or ') in line %d",
							lineNumber);
					images.clear();
	...
	private boolean parseforImagesGetName(String current, boolean inString, String[] possibleImage,
			String[] stringType) {
...
//</editor-fold>
	...
	private void initFileMenu() throws NoSuchMethodException {
		...
		_fileMenu.add(createMenuItem("Export as jar", KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_J, scMask),
				new FileAction(FileAction.ASJAR)));

		_fileMenu.add(createMenuItem("Export as runnable jar",
				KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_J, InputEvent.SHIFT_DOWN_MASK | scMask),
				new FileAction(FileAction.ASRUNJAR)));

		jmi = _fileMenu.add(createMenuItem(_I("menuFileCloseTab"),
				KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_W, scMask), new FileAction(FileAction.CLOSE_TAB)));
...
//TODO restart IDE
		 ...
		 * InputEvent.SHIFT_DOWN_MASK | InputEvent.ALT_DOWN_MASK), new
		 * FileAction(FileAction.RESTART)));
		 */
...
//</editor-fold>
	...
	private void initEditMenu() throws NoSuchMethodException {
		...
		_editMenu.add(createMenuItem(_I("menuEditCopy"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_C, scMask),
		...
		_editMenu.add(createMenuItem("Copy line",
				...
				new EditAction(EditAction.COPY)));
		_editMenu.add(createMenuItem(_I("menuEditCut"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_X, scMask),
		...
		_editMenu.add(createMenuItem("Cut line",
				...
				new EditAction(EditAction.CUT)));
		_editMenu.add(createMenuItem(_I("menuEditPaste"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_V, scMask),
				new EditAction(EditAction.PASTE)));
		_editMenu.add(createMenuItem(_I("menuEditSelectAll"),
				KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_A, scMask), new EditAction(EditAction.SELECT_ALL)));

		_editMenu.addSeparator();
		JMenu findMenu = new JMenu(_I("menuFind"));
		findMenu.setMnemonic(KeyEvent.VK_F);
		findMenu.add(createMenuItem(_I("menuFindFind"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_F, scMask),
				new FindAction(FindAction.FIND)));
		findMenu.add(createMenuItem(_I("menuFindFindNext"),
				KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_G, scMask), new FindAction(FindAction.FIND_NEXT)));
		...
		_editMenu.addSeparator();
		_editMenu.add(createMenuItem(_I("menuEditIndent"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_TAB, 0),
...
//      _searchField.requestFocus();
		...
		boolean findNext(String str) {
			if (getCurrentCodePane() != null) {
				return _find(str, getCurrentCodePane().getCaretPosition() + str.length(), true);
			}
			return false;
		}
...
//    }
	...
	private void initRunMenu() throws NoSuchMethodException {
		...
		_runMenu.add(createMenuItem(_I("menuRunRun"), KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_R, scMask),
				new RunAction(RunAction.RUN)));
		_runMenu.add(createMenuItem(_I("menuRunRunAndShowActions"),
				KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.ALT_DOWN_MASK | scMask),
...
//TODO Message Area LineBreak
	...
	private void initToolMenu() throws NoSuchMethodException {
		...
		_toolMenu.add(createMenuItem(_I("menuToolExtensions"), null, new ToolAction(ToolAction.EXTENSIONS)));

		_toolMenu.add(createMenuItem("Pack Jar with Jython", null, new ToolAction(ToolAction.JARWITHJYTHON)));

		_toolMenu.add(createMenuItem(_I("menuToolAndroid"), null, new ToolAction(ToolAction.ANDROID)));
	}
...
//    ADBScreen.stop();
	...
	private void initHelpMenu() throws NoSuchMethodException {
		...
		_helpMenu.add(createMenuItem(_I("menuHelpQuickStart"), null, new HelpAction(HelpAction.QUICK_START)));
		_helpMenu.addSeparator();

		_helpMenu.add(createMenuItem(_I("menuHelpGuide"), null, new HelpAction(HelpAction.OPEN_DOC)));
//    _helpMenu.add(createMenuItem(_I("menuHelpDocumentations"),
//            null, new HelpAction(HelpAction.OPEN_GUIDE)));
		_helpMenu.add(createMenuItem(_I("menuHelpFAQ"), null, new HelpAction(HelpAction.OPEN_FAQ)));
		_helpMenu.add(createMenuItem(_I("menuHelpAsk"), null, new HelpAction(HelpAction.OPEN_ASK)));
		_helpMenu.add(createMenuItem(_I("menuHelpBugReport"), null, new HelpAction(HelpAction.OPEN_BUG_REPORT)));

//    _helpMenu.add(createMenuItem(_I("menuHelpTranslation"),
//            null, new HelpAction(HelpAction.OPEN_TRANSLATION)));
		_helpMenu.addSeparator();
		_helpMenu.add(createMenuItem(_I("menuHelpHomepage"), null, new HelpAction(HelpAction.OPEN_HOMEPAGE)));
	...
	class HelpAction extends MenuAction {
		...
		public void doCheckUpdate(ActionEvent ae) {
			if (!checkUpdate(false)) {
				...
				String updMsg = newBuildAvailable != null
						? (newBuildAvailable ? _I("msgUpdate") + ": " + newBuildStamp : _I("msgNoUpdate"))
						: _I("msgUpdateError");
				JOptionPane.showMessageDialog(null, updMsg, Commons.getSXVersionIDE(), msgType);
			}
		}
...
//    toolbar.add(jcSearchField);
	...
	class ButtonInsertImage extends ButtonOnToolbar {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			if (!name.isEmpty()) {
				final File newFile = new File(imgFile.getParentFile(),
						name + "." + FilenameUtils.getExtension(imgFile.getName()));
				if (!newFile.exists()) {
					if (imgFile.renameTo(newFile)) {
						imgFile = newFile;
					}
				} else {
					final String msg = String.format("%s already exists - stored as %s", newFile.getName(),
							imgFile.getName());
	...
	class ButtonShowIn extends ButtonSubregion {
		...
		public boolean shouldRun() {
			...
			if (item.startsWith("Pattern")) {
				item = "m = null; r = #region#; " + "if (r != null) m = r.exists(new " + item + ", 0); "
						+ "if (m != null) m.highlight(2); else print(m);";
			} else if (item.startsWith("\"")) {
				item = "m = null; r = #region#; " + "if (r != null) m = r.exists(" + item + ", 0); "
						+ "if (m != null) m.highlight(2); else print(m);";
			} else {
				item = "";
			}
			...
			if (isRunning()) {
				...
				new Thread(() -> {
					try {
						...
						if (!actions.isEmpty()) {
							List<String> actionStrings = actions.stream().map((a) -> a.generate(generator))
...
//</editor-fold>
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						exitValue = runner.runScript(context.getFile().getAbsolutePath(), Commons.getUserArgs(),
								runOptions);
					} catch (Exception e) {
	...
	private AIChatPanel chatArea = new AIChatPanel();
